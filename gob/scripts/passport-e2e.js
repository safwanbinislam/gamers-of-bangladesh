...
async function main() {
  ...
  const server = spawn(
    ...
    {
      cwd: "d:\\Gamers of Bangladesh\\gob",
  ...
  if (!up) {
    console.error("FATAL: dev server did not become ready");
    server.kill();
    process.exit(1);
  }
  ...
  try {
    ...
    console.log("Cookies obtained. owner len=" + ownerCookie.length + " viewer len=" + viewerCookie.length);

    console.log("\n== 1 PUBLIC /players/[owner] ==");
    const pub = await httpGet(`${BASE}/players/${OWNER_ID}`);
    assert(pub.status === 200, "status 200", String(pub.status));
    assert(pub.body.includes("passport_owner"), "username passport_owner");
